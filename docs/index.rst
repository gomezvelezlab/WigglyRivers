.. WigglyRivers documentation master file, created by
   sphinx-quickstart on Wed Feb 12 23:29:28 2025.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

WigglyRivers documentation
==========================
...
The synthetic river transect examples are generated using idealized Kinoshita-type curves, implemented in the code, and using the `meanderpy <https://github.com/zsylvester/meanderpy>`_ package coded by Sylvester, Durkin, and Covault (2019). The natural river transect example uses the information from the `High-Resolution National Hydrography Dataset (NHDPlus HR) <https://www.usgs.gov/core-science-systems/ngp/national-hydrography/nhdplus-high-resolution>`_ to extract the river transects and assess the meandering features.
...
1. **Kinoshita Examples** (`Kinoshita_Examples.ipynb`): 
   ...
   - Generation of Kinoshita curves with different parameters
   - Curvature and direction-angle calculations
   - Wavelet analysis of river planforms
   - Meander identification and characterization

2. **Meanderpy River Example** (`Meanderpy_River_Example.ipynb`):
   Shows how to use the `meanderpy` package integration to:
   
   - Generate synthetic meandering rivers
...
3. **NHDPlus HR Data Extraction** (`Extract_NHDPlus_HR_Data.ipynb`):
   ...
   - Extract river data from the NHDPlus HR dataset
   - Process and analyze real river transects
   - Identify and characterize natural meanders
   - Visualize results with interactive plots

These notebooks can be found in the `examples/new_user_workflow/` directory of the repository.
...
This package has a few requirements. I encourage using a virtual environment of `Anaconda 3 <https://www.anaconda.com/products/individual>`_ with Python 3.10 or higher. The virtual environment creation can be seen `here <https://docs.conda.io/projects/conda/en/latest/user-guide/tasks/manage-environments.html>`_. Below, we list the process to create a virtual environment and install the requirements for the package.
...
- If you have problems with ``geopandas`` look at `this website <https://wilcoxen.maxwell.insightworks.com/pages/6373.html#:~:text=It%20has%20complex%20links%20to,between%2010%20and%2030%20minutes.>`_.
...
- There is a known issue with ``plotly<=5.15`` where plotting MAPBOX with the interactive widget will prompt the following error message:
  ...
  There is a temporary fix to this issue given in the following `GitHub issue webpage <https://github.com/plotly/plotly.py/issues/2570>`_ that requires the use of the function below and restart the kernel.
