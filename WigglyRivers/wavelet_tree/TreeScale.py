...
class RiverTreeScales:
    ...
    def __init__(self, trees=None, uid=0, logger=None):
        ...
        self.node_ids = {}
        self.metrics = [
            'peak_col', 'peak_row', 'peak_pwr',
            's_c', 'scales_c', 'wavelength_c', 'x_c', 'y_c',
            'r_x', 'r_y', 'x_1', 'x_2', 'x_3', 'y_1', 'y_2', 'y_3',
            'cc_x', 'cc_y', 'beg_idx', 'end_idx', 'mid_idx', 'radius',
    ...
    def build_trees_from_tree_scales_dict(self, tree_scales):
        ...
        while len(branch_ids) > 0:
            ...
            root_node = self._build_tree_from_linking_branches(
                tree_scales, linking_branch, tree_id=tree_id,
    ...
    def _build_tree_from_linking_branches(self, tree_scales, linking_branch,
                                          tree_id=0, meander_count=0):
        """
        ...
        for i_b, b in enumerate(linking_branch):
            ...
            for i_l, level in enumerate(levels_root_leaf):
                ...
                # Extract Information
                meander_in_level = tree_scales[
                    'meander_in_level_root_leaf'][b]
                ...
                if i_l == 0 and i_b == 0: # Root Node
                    node = self._create_tree_node(
                        tree_id, node_id, metrics, node_type='root', 
                        is_meander=is_meander, meander_id=meander_id)
                else:
                    ...
                    if i_l != len(levels_root_leaf)-1: # Parent Node
                        node = self._create_tree_node(
                            tree_id, node_id, metrics, parent=parent,
                            node_type='parent',
                            is_meander=is_meander, meander_id=meander_id)
                    else: # Leaf node
                        node = self._create_tree_node(
    ...
    def _create_tree_node(self, tree_id, node_id, metrics, parent=None,
                          node_type='root', is_meander=0, meander_id=-1):
        """
            Create tree node
        """
        # Select type
        ...
        node = Node(node_name, parent=parent,
                    tree_id=tree_id,
                    node_id=node_id, root_node=root_node,
                    parent_node=parent_node, leaf_node=leaf_node,
                    is_meander=is_meander, meander_id=meander_id,
                    **metrics)
        return node
    ...
    def _extract_metrics(self, tree_scales, branch_id, level):
        ...
        """
        metrics_values = {i: tree_scales[i][branch_id][level]
    ...
    def select_node(self, tree_id, node_id):
        ...
        selected_node = list(
            PreOrderIter(root_node, filter_=lambda n: n.node_id == node_id))[0]
    ...
    def filter_nodes(self, tree_ids=None, key=None, value=None):
        ...
        for tree_id in tree_ids:
            nodes[tree_id] = list(
                PreOrderIter(self.trees[tree_id],
    ...
    def compile_database(self):
        ...
        for tree_id in self.tree_ids:
            if tree_id == 0:
                ...
                compiled_database = pd.concat([compiled_database,
                                                self.database[tree_id]],
                                                ignore_index=True)
    ...
    def save_tree_figure(self, tree_id, path_img='', file_name='tree.png'):
        ...
        root_node = self.trees[tree_id]
        DotExporter(root_node).to_picture(
            f'{path_img}{file_name}')
        return
    ...
    @staticmethod
    def _prune_by_sinuosity(node, already_extracted=[], sinuosity_var='sn',
    ...
    @staticmethod
    def _prune_by_width(node, already_extracted=[], width_var='w_m',
                        compare_var='wavelength_c',
                        gamma=10, additional_threshold=1e6,
    ...
    @staticmethod
    def _prune_by_peak_power(node, already_extracted=[], peak_power_var='peak_pwr', 
                            sign_var='direction_node_to_parent',
                            characteristic_length=1e6, compare_var='radius'):
        """
        ...
        sign_children = [
            child.__dict__[sign_var] for child in children_nodes]
