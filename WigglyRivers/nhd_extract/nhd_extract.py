...
class ExtractNHDPlusHRData:
    ...
    def __init__(self, path_output, name='file', comid_name='NHDPlusID',
                 logger=None, **kwargs):
        """
        Class constructor
        """
    ...
    @staticmethod
    def _organize_coordinates(coords_dict, comid='NHDPlusID', neglect=None,
                              comid_list=None):
        """
    ...
    def get_data_from_nhd_gbd(
            self, file_data, flowlines='NHDFlowline', comid='NHDPlusID',
            tables=None, projection=None):
        """
        ...
        if ext == 'zip':
            self.logger.warning(' WARNING: GDB in zip file, loading from zip'
                                ' file')
        ...
        for i in range(shapefile.shape[0]):
            ...
            if projection is not None:
                ...
                coords_projected[i], coords_all_comid_projected = \
                    self._get_coordinates(bounds_proj,
                                          shapefile_projected[comid].values[i])
        ...
        coords_dict = {
            ...
            'xup_deg': coords[:, 0], 'yup_deg': coords[:, 1],
            'xm_deg': coords[:, 2], 'ym_deg': coords[:, 3],
            'xdown_deg': coords[:, 4], 'ydown_deg': coords[:, 5],
        }
        if projection is not None:
            coords_dict.update({
                'xup_m': coords_projected[:, 0], 'yup_m': coords_projected[:, 1],
                'xm_m': coords_projected[:, 2], 'ym_m': coords_projected[:, 3],
                'xdown_m': coords_projected[:, 4], 'ydown_m': coords_projected[:, 5],
            })
        ...
        for i_t, t in enumerate(tables):
            ...
            if len(t_n) == 0:
                ...
                # Extract data
                data_t_n = {i: np.zeros(comid_t_n.shape)*np.nan
                            for i in columns_t_n}
            ...
            try:
                table = pd.merge(table, t_n[cols_to_use], left_index=True,
        ...
        if self.save_format != 'hdf5':
            self.logger.warning(f"  WARNING: The coordinates are always saved"
                                f" as 'hdf5'")
        self.logger.info(f' Saving {path_output}/{name_out}')
        FM.save_data(
            save_data_coord, path_output, name_out)
        ...
        if projection is not None:
            ...
            if self.save_format != 'hdf5':
                self.logger.warning(f"  WARNING: The coordinates are always "
                                    f"saved as 'hdf5'")
            self.logger.info(f' Saving {path_output}/{name_out}')
            FM.save_data(
                save_data_coord, path_output, name_out)
